<template>
  <section>
      ...
      <b-table
          ...
          <b-table-column field="rolNombre" label="Rol" sortable v-slot="props">
              {{ props.row.rolNombre }}
          </b-table-column>
    ...
    methods: {
      ...
      obtenerUsuarios() {
          ...
          .then(async resultado => {
              let roles = await HttpService.obtener("obtener_roles.php");
              let rolesMap = {};
              roles.forEach(role => {
                  rolesMap[role.id] = role.nombre;
              });

              this.usuarios = resultado.map(usuario => {
                  return {
                      ...usuario,
                      rolNombre: rolesMap[usuario.role_id]
                  };
              });
              this.cargando = false;
          })
