  ...
  methods: {
    ...
    if (this.insumosAnteriores.length > 0) {
      this.insumosAnteriores.forEach((insumo) => {
        let subtotal = parseFloat(insumo.cantidad) * parseFloat(insumo.precio);
        if (insumo.adicional !== 0 && insumo.adicional !== null && insumo.adicional !== undefined) {
          subtotal += parseFloat(insumo.adicional);
        }
        totalAnterior += subtotal;
      });
    }
