...
function verificarTablas() {
    ...
    $sentencia  = $bd->query("SELECT COUNT(*) AS resultado FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME = 'killabar_botanero_ventas'");
...
function obtenerRoles() {
  $bd = conectarBaseDatos();
  $sentencia = $bd->query("SELECT id, nombre FROM roles");
  return $sentencia->fetchAll();
}
...
function registrarUsuario($usuario) {
  $bd = conectarBaseDatos();
  $sentencia = $bd->prepare("INSERT INTO usuarios (correo, nombre, telefono, password, role_id) VALUES(?,?,?,?,?)");
  return $sentencia->execute([$usuario->correo, $usuario->nombre, $usuario->telefono, $usuario->password, 1]);
}
...
function conectarBaseDatos() {
	$host = "killabar.com";
	$db   = "killabar_botanero_ventas";
	$user = "killabar_cristhian";
	$pass = "cristian2305:v";
	$charset = 'utf8mb4';
